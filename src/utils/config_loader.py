...
class Config:
    ...
    DEBUG_MODE: bool = os.getenv(
        "DEBUG_MODE", "False").lower() in ("true", "1", "yes")
    ...
    DATABASE_FILE: str = os.getenv(
        "DATABASE_FILE", "bot_database.db")  # SQLite file path

    # Webhook configuration
    USE_WEBHOOK: bool = os.getenv(
        "USE_WEBHOOK", "False").lower() in ("true", "1", "yes")
    ...
    def validate(cls):
        ...
        if cls.DATABASE_TYPE not in ("sqlite", "postgres"):
            raise ValueError(
                "DATABASE_TYPE must be either 'sqlite' or 'postgres'.")
